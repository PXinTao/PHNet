...
def read_from_pair_txt(path, filename):
    ...
    filenames = [(os.path.join(path, c[0]),
                  os.path.join(path, c[1]),
                  os.path.join(path, c[2]),
                  os.path.join(path, c[3]),
                  os.path.join(path, c[4]),
                  os.path.join(path, c[5]),
                  os.path.join(path, c[6])) for c in filenames]
    return filenames
    ...
    def __init__(self, root, flag='train', VOC=False, transform=None):
        if flag == 'train':
            if VOC:
                ...
                filenames = read_from_pair_txt(root['BSDS'], 'train_pair_c.lst')
            self.im_list = [im_name[0] for im_name in filenames]
            self.gt_list = [im_name[1] for im_name in filenames]

        elif flag == 'test':
            self.im_list = glob.glob(os.path.join(root['BSDS'], 'test', '*.jpg'))
