...
if __name__ == '__main__':
    ...
    # net = model.Detector(cfgs).eval()
    net = deep_xy.VisualNet().eval()
    net.load_state_dict(torch.load('./checkpiont/9_model_AdamW_01725_ep10.pth',map_location='cuda:0'))
    ...
    for i, data in enumerate(dataloader):
        with torch.no_grad():
            ...
            # prop
            if not os.path.exists('./ep10_test/1X/'):
                os.makedirs('./ep10_test/1X/')
            ...
            cv2.imwrite('./ep10_test/1X/' + os.path.basename(name_list[i]).split('.')[0] + '.png', prediction * 255)
